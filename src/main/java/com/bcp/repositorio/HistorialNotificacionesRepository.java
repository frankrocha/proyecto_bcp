...
public interface HistorialNotificacionesRepository  extends JpaRepository<HistorialNotificaciones, Integer>{
	...
	@Query("Select x from HistorialNotificaciones x where x.cliente.idCliente = :var_cli  ORDER BY x.idHistorialNotificaciones DESC ")
	public abstract List<HistorialNotificaciones> listaPorCliente(@Param(value = "var_cli") int idCliente);

	  
	@Query("Select x from HistorialNotificaciones x where x.cliente.idCliente = :var_cli  ORDER BY x.idHistorialNotificaciones DESC")
    public abstract List<HistorialNotificaciones> listaPorCliente2(@Param(value = "var_cli") int idCliente);	
